...
SOURCE=BASE.parent/'models/spider-original.glb'
OUTPUT=BASE/'spider-rigged.glb'
import hashlib
assert hashlib.sha256(SOURCE.read_bytes()).hexdigest()=='756b617f8392c3a43ac8432dee4182ee52fd51cbf20803c52e90c6877ed3db1f','Unexpected source mesh'
...
remaining={i for i,p in enumerate(unique) if abs(p[0])>.32}
components=[]
while remaining:
...
arm_data=bpy.data.armatures.new('September9SpiderSkeleton')
arm=bpy.data.objects.new('September9SpiderRig',arm_data)
...
bone('Abdomen',(0,.04,body_z),(0,.57,body_z+.09),'Body')
bone('Head',(0,-.28,body_z),(0,-.53,body_z-.015),'Body')
bone('ContactBase',(0,-.52,.40),(0,-.54,.40),'Head',False)
bone('ContactTip',(0,-.68,.21),(0,-.72,.21),'Head',False)
for sign,label in [(-1,'L'),(1,'R')]:
    bone('Fang_'+label,(sign*.073,-.48,.43),(sign*.08,-.67,.18),'Head')
    bone('Palp_'+label,(sign*.165,-.40,.43),(sign*.23,-.61,.19),'Head')
for leg in leg_data:
    j=leg['joints'];names=[]
...
print('Eight fitted leg components ready',flush=True)
...
for vi,p in enumerate(positions):
    ...
    if label is None and ax>.17 and p[1]<.36:
    ...
    elif p[1]<-.47 and p[2]<.46 and ax<.145:
        weights['Fang_'+('L' if p[0]<0 else 'R')]=1
    elif p[1]<-.43 and p[2]<.46 and .145<=ax<.29:
...
print('Weights ready',len(weighted),flush=True)
mesh.parent=arm
