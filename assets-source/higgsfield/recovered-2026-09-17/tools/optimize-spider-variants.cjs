...
function options(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === '--variant') { o.variant=argv[++i]; if(!['mobile1024','hd2048'].includes(o.variant)) throw Error('Invalid variant'); }
...
async function resizeTextures(json, views, maxSize, sharp, allowWebp) {
  ...
  for (const [i, image] of (json.images || []).entries()) {
    ...
    const convertWebp = allowWebp && coreRefs.length > 0 && !hasAlternative;
...
async function main() {
  ...
  if (!o.inspectOnly) {
    ...
    for (const [name, cap] of [['mobile1024', 1024], ['hd2048', 2048]].filter(([name])=>!o.variant||o.variant===name)) {
  ...
  artifacts.push({ filename: `${stem}.${o.variant||'both'}.report.json`, bytes: Buffer.from(jsonText(report)) });
  const checksums = artifacts.map(a => `${sha(a.bytes)}  ${a.filename}`).join('\n') + '\n';
  artifacts.push({ filename: `${stem}.${o.variant||'both'}.SHA256SUMS`, bytes: Buffer.from(checksums) });
