...
for vi,v in enumerate(points):
 ...
 if slug=='ranger' and v[2]<.01 and v[1]>.34 and cape[vi]:
...
labels=mask[tri];mixed=labels.any(axis=1)&~labels.all(axis=1);centers=ps[tri].mean(axis=1);grip=bp[index['LeftHand']];bridge=mixed&(np.linalg.norm(centers-grip,axis=1)>.09)
...
removed=np.where(bridge|cape_bridge)[0];meta['derivativeSeamCuts']={'reason':'Separate provider-fused bow/string from sleeve and cape from forearm to prevent stretched bridge faces','triangleIds':removed.tolist(),'bowBridgeCount':int(bridge.sum()),'capeSleeveBridgeCount':int(cape_bridge.sum()),'sourceTriangles':len(tri)};tri=tri[~(bridge|cape_bridge)]
...
report={'input':str(SOURCE.relative_to(BASE)),'inputSHA256':hashlib.sha256(raw).hexdigest(),'output':str(OUTPUT.relative_to(BASE)),'outputSHA256':hashlib.sha256(output).hexdigest(),'sourceVertexGeometryUntouched':True,'topologyChangedOnlyForRecordedSeamCuts':True,'originalBinaryPrefixPreserved':True,'vertices':len(ps),'triangles':len(tri),'virtualWeldedVertices':count,'hardPinnedVertices':int(fixed.sum()),'smoothingPasses':100,'equipmentVerticesRigidHand':len(equipment),'derivativeSeamCuts':meta['derivativeSeamCuts'],'weightSumMaxError':float(np.max(np.abs(ws.sum(axis=1)-1))),'method':'Continuous vertical torso/hood field with fitted arm capsules, preserved rigid scythe anchors and seam-aware topological smoothing. Robed characters lower garment follows Hips; fitted arm capsule fields and seam-welded smoothing avoid transferred distant hand/leg islands.'}
