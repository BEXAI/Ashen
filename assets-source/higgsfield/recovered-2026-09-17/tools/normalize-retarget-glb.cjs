...
const DEPS = process.env.ASHEN_ASSET_DEPS || process.cwd();
...
function args(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '--force') o.force = true;
    ...
    else if (['--clip', '--profile', '--metallic', '--roughness', '--contact-base', '--contact-tip', '--contact-bone', '--deps-root', '--provenance-json', '--clip-duration', '--muzzle'].includes(a)) {
      ...
      else if (a === '--muzzle') o.muzzle = v.split(',').map(Number);
  ...
  if (o.clips.some(c => c.name === 'turn')) throw Error('Own idle is preserved automatically; turn belongs to runtime actor yaw.');
...
function normalizeMaterials(json, o) {
  return (json.materials || []).map((m, index) => {
    ...
    if (!m.pbrMetallicRoughness.metallicRoughnessTexture) {m.pbrMetallicRoughness.metallicFactor = o.metallic; m.pbrMetallicRoughness.roughnessFactor = o.roughness;}
...
function addSockets(builder, measurements, o) {
  ...
  if(o.muzzle){hand.children.push(builder.json.nodes.length);builder.json.nodes.push({name:'EquippedMuzzle',translation:o.muzzle,extras:{purpose:'Measured original embedded equipment emission point'}});}
...
async function main() {
  ...
  if (targetRig && !o.materialsOnly) {
    ...
    if (idle && !o.clips.some(c => c.name === 'idle') && clipExtent(idle).duration > 0) {
  ...
  // Static source poses are not motion; replace them with the genuine idle donor.
  if (o.clips.some(c => c.name === 'idle')) builder.json.animations = builder.json.animations.filter(a => !/idle/i.test(a.name) && a.samplers.some(s => {const t=builder.json.accessors[s.input];return t.count>1 && (t.max?.[0]??0)>(t.min?.[0]??0);}));
