...
function sampleSkinnedBounds(document, THREE, sampleCount = 17, exclusions = {}) {
  ...
  function pose(tracks, time) {
    ...
    const bounds = box();let minimumVertex=null;
    for (const p of primitives) {
      ...
      for(let v=0;v<p.positions.length/3;v++) {
        ...
        if(oy<bounds.min[1])minimumVertex={node:nodes[p.node].getName(),vertex:v,restPosition:[x,y,z],posedPosition:[ox,oy,oz],influences:p.sets.flatMap(set=>Array.from({length:4},(_,k)=>({joint:p.skin?nodes[p.skin.joints[set.joints[v*4+k]]].getName():null,weight:set.weights[v*4+k]}))).filter(x=>x.weight>0)};
        expand(bounds,ox,oy,oz);
      }
    }
    const contacts={};for(let i=0;i<nodes.length;i++)if(['ContactBase','ContactTip','Hips','LeftFoot','RightFoot','LeftToeBase','RightToeBase'].includes(nodes[i].getName()))contacts[nodes[i].getName()]=new THREE.Vector3().setFromMatrixPosition(world[i]).toArray();
    return { time, bounds: completed(bounds), minimumVertex, keyNodePositions: contacts };
  }
