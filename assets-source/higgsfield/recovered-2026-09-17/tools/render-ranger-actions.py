...
if gait:views=[(name,(1.5,-3,1.4)) for name in ['idle_0.5','forward_0.5','side_0.725','diagonal_0.725','overhead_0.725','dodge_0.5','hit_0.5','death_0.85']]
for name, offset in views:
    if gait:
        clip,frame=name.rsplit('_',1)
        rig=next(o for o in scene.objects if o.type=='ARMATURE')
        rig.animation_data.action=bpy.data.actions[clip]
        rig.animation_data.action_slot=bpy.data.actions[clip].slots[0]
        rng=bpy.data.actions[clip].frame_range
        scene.frame_set(int(rng[0]+(rng[1]-rng[0])*float(frame)))
    if name=='top':
...
if gait:
    ...
    sheet=np.concatenate([np.concatenate(panels[i:i+4],axis=1) for i in [4,0]],axis=0)
    ...
    composed.filepath_raw=str(output/'action-contact-sheet.png');composed.file_format='PNG';composed.save()
