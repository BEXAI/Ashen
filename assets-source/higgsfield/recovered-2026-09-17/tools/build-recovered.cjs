...
async function main(){
 ...
 const selected=process.argv.slice(2),summaryPath=path.join(base,'runtime-build-summary.json');let summary={schemaVersion:1,models:[]},curves={},entries={};
 try{summary=await read(summaryPath);curves=await read(path.join(base,'runtime-grounding.json'));entries=await read(path.join(base,'manifest-entries.json'));}catch{}
 for(const source of inventory.assets.toSorted((a,b)=>selected.length?selected.indexOf(a.slug)-selected.indexOf(b.slug):0)){
  ...
  let input=path.join(base,'derived',slug+'-rigged.glb');try{await fs.access(input);}catch{input=path.join(base,'models',slug+'-original.glb');}
  ...
  summary.updatedAtUTC=new Date().toISOString();await fs.writeFile(summaryPath,json(summary));await fs.writeFile(path.join(base,'runtime-grounding.json'),JSON.stringify(curves)+'\n');await fs.writeFile(path.join(base,'manifest-entries.json'),json(entries));
