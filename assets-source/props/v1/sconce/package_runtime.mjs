...
const out=process.env.SCONCE_OUT||path.dirname(fileURLToPath(import.meta.url)),toktx=process.env.ASHEN_TOKTX,ktx=process.env.ASHEN_KTX;
...
for(const lod of [0,1,2,'combined']){
 const prefix=lod==='combined'?'sconce-pilot':`sconce-lod${lod}`;
 const rawPath=path.join(out,`${prefix}-fallback.glb`),raw=fs.readFileSync(path.join(out,lod==='combined'?'sconce-combined-baked.glb':`sconce-lod${lod}-baked.glb`));
 ...
 if(lod==='combined'){const fdec=await io.readBinary(fb);await fdec.transform(dequantize());fdec.disposeExtension('EXT_meshopt_compression');await io.write(path.join(out,'sconce-pilot-fallback-decoded-qa.glb'),fdec);}
...
manifest.mobileEstimatedRgba8MipBytes=(512*512+2*256*256)*4*4/3;
