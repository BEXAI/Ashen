...
export class GameEngine {
 ...
 private renderHistoryContext(at:number){
  ...
  return {
   ...
   characterAssets:{states:{...this.characters?.status,...this.roster?.status},tiers:Object.fromEntries(Object.entries(this.characters?.sources??{}).map(([skin,source])=>[skin,source.tier])),overlap:this.characters?.overlap??null},
 ...
 private releaseBolt(actor:Actor,swing:Swing,id:RosterId,enemy?:Enemy){
  ...
  const wall=this.obstruction(start,end);if(Number.isFinite(wall))end.lerpVectors(start,end,Math.max(0,wall-.04)/12);
  if(enemy){
