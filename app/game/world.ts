...
import type { HiggsfieldTarget } from './higgsfield-environment';
...
export type WorldScene={scene:T.Scene,sun:T.DirectionalLight,hero:Actor,keeper:Actor,shrines:{id:string,group:T.Group,light:T.PointLight,flame:T.Mesh}[],chests:{id:string,group:T.Group,lid:T.Group}[],fires:{light:T.PointLight,flame:T.Mesh}[],water:T.ShaderMaterial,embers:T.Points,colliders:{x:number,z:number,r:number}[],materials:T.Material[],surfaces:Surface[],sky:T.Mesh,reflector:Reflector,architecture:T.Mesh,gates:T.Mesh[],higgsfieldTargets:HiggsfieldTarget[]};
...
export function createWorld(p:Progress,quality:string):WorldScene {
 seed=72139;
 const scene=new T.Scene();scene.background=new T.Color('#06090d');scene.fog=new T.FogExp2('#11151b',.022);
 ...
 function worldUV(g:T.BufferGeometry){
  ...
  for(let i=0;i<pos.count;i++){
   if(Math.abs(n.getY(i))>.5)uv.setXY(i,pos.getX(i)/4,pos.getZ(i)/4);
   else if(Math.abs(n.getX(i))>.5)uv.setXY(i,pos.getZ(i)/4,pos.getY(i)/4);
   else uv.setXY(i,pos.getX(i)/4,pos.getY(i)/4);
  }
  return g;
 }
 const geometry:T.BufferGeometry[]=[];
 function wall(x:number,y:number,z:number,w:number,h:number,d:number){
  const g=new T.BoxGeometry(w,h,d);g.translate(x,y,z);worldUV(g);geometry.push(g);
 }
 const colliders:{x:number,z:number,r:number}[]=[];
 const higgsfieldTargets:HiggsfieldTarget[]=[];
 function assetRoot(kind:HiggsfieldTarget['kind'],room:typeof ROOMS[number],x:number,y:number,z:number){
  const root=new T.Group();root.position.set(x,y,z);scene.add(root);
  const target:HiggsfieldTarget={kind,root,fallback:[],room:room.id,roomZ:room.z,roomDepth:room.depth};
  higgsfieldTargets.push(target);return target;
 }
 for(const room of [...ROOMS,...CORRIDORS]){
  const g=new T.PlaneGeometry(room.width,room.depth);g.rotateX(-Math.PI/2);g.translate(room.x,0,room.z);worldUV(g);
  mesh(g,floor,0,0,0);
 ...
 ROOMS.forEach((room,i)=>{
  for(const [edge,open] of [[room.z-room.depth/2,i<ROOMS.length-1],[room.z+room.depth/2,i>0]] as [number,boolean][]){
   if(open){
    ...
    const target=assetRoot('arch',room,0,0,edge);
    const arch=mesh(new T.TorusGeometry(3.2,.32,8,24,Math.PI),stone,0,4,0,target.root);arch.scale.z=1.6;
    for(const side of [-1,1])mesh(new RoundedBoxGeometry(.7,4,1,2,.1),stone,side*3.25,2,0,target.root);
    target.fallback=[...target.root.children];
  ...
  for(let z=room.z-room.depth/2+3;z<room.z+room.depth/2;z+=6){
   ...
   for(const side of [-1,1]){
    const target=assetRoot('pillar',room,side*(room.width/2-.35),0,z);
    mesh(new RoundedBoxGeometry(.7,5.2,.9,2,.08),stone,0,2.6,0,target.root);
    mesh(new RoundedBoxGeometry(1.1,.35,1.2,2,.06),stone,0,5.05,0,target.root);
    target.fallback=[...target.root.children];
   }
  }
 });
 ...
 for(const room of ROOMS){
  const pieces:T.Matrix4[]=[],targets:HiggsfieldTarget[]=[];
  for(const side of [-1,1]){
  for(let z=room.z-room.depth/2+1.5;z<room.z+room.depth/2-1;z+=2.1){
   ...
   const target=assetRoot('trim',room,dummy.position.x,.025,z);target.root.rotation.y=Math.PI/2;target.root.scale.set(.98,.55/.35,1.1);targets.push(target);
  ...
  targets.forEach(target=>{target.fallback=[edging];});
 }
 scene.add(new T.HemisphereLight('#718399','#18110b',.5));
 ...
 const fires:{light:T.PointLight,flame:T.Mesh}[]=[];
 function fire(parent:T.Object3D,x:number,y:number,z:number,size=1,color='#ffab55'){
  ...
  const flame=mesh(new T.SphereGeometry(.15,8,6),new T.MeshBasicMaterial({color,toneMapped:false}),x,y+.4,z,parent);flame.scale.set(.7*size,2.2*size,.7*size);flame.userData.baseScale=flame.scale.y;fires.push({light,flame});return {light,flame};
 }
 ROOMS.forEach(room=>{for(const side of [-1,1]){
  const x=side*(room.width/2-1),z=room.z;
  const bracket=mesh(new T.CylinderGeometry(.08,.1,.9,8),iron,x,2.4,z);bracket.rotation.z=side*.35;
  const bowl=mesh(new T.CylinderGeometry(.3,.18,.25,10),iron,x,2.9,z),f=fire(scene,x,3,z,1.4);
  const target=assetRoot('torch-sconce',room,side*(room.width/2-.45),3.015,z);target.root.rotation.y=-side*Math.PI/2;
  target.fallback=[bracket,bowl];target.flame=f.flame;target.light=f.light;
 }});
 ...
 const shrines=WORLD.shrines.map(s=>{
  ...
  const fallback=[...group.children],room=ROOMS.find(room=>room.id===s.id)!;
  const f=fire(group,0,1.5,0,1,p.shrines.includes(s.id)?'#8cd6ec':'#d08239');
  higgsfieldTargets.push({kind:'ember-shrine',root:group,fallback,room:room.id,roomZ:room.z,roomDepth:room.depth,flame:f.flame,light:f.light});return {id:s.id,group,...f};
 });
 ...
 return {scene,sun,hero,keeper,shrines,chests,fires,water,embers,colliders,materials,surfaces:[{material:floor,family:'floor',repeat:1},{material:stone,family:'wall',repeat:1}],sky,reflector,architecture,gates,higgsfieldTargets};
}
