...
The live game integrates the September 8 screenshot as six selectable heroes, ten enemies, sixteen journal/selection portraits, and the original crown film. Fourteen characters are Higgsfield reconstructions. Ogre and goblin are locally authored stylized interpretations after the provider rejected the inputs. Weighted animation and restored equipment are described in the source inventory; this is not an exact photoreal reconstruction guarantee.
...
Performance mode caps roster textures to 512 pixels before GPU upload, closing superseded decoded bitmaps. Compared with a 1024-pixel texture of the same aspect/format, this quarters the texture pixel allocation. It does not reduce network download size or eliminate the transient original decode. Mobile Auto continues to use 1024-pixel assets, while desktop uses 2048. New roster meshes have a single geometry level; Ogre/Goblin retain multiple materials. These are limits, not claims of measured phone performance. Diagnostics identify the actual roster characters and tiers.
