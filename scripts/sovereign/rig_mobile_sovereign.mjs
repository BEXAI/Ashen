 ...
 * Existing game master, bones, clips and weapon are preserved. No source game writes.
 */
...
function transform(v,name){
 ...
 if(c==='cape')return [x*1.8,interp(y,[[0,0],[1.56,2.15]]),z*1.3];
 if(c==='body'){
...
function influence(v,name,mapped){
 ...
 if(c==='arm'){
  ...
  if(/pauldron|arm shell|shoulder defense/.test(name))return [[arm,1]];
...
const final={bytes:result.length,triangles,materials:doc.getRoot().listMaterials().length,meshes:doc.getRoot().listMeshes().length,weaponTriangles,bones:skin.listJoints().map(j=>j.getName()),clips:originalClipSignatures,validation:report.issues,weightedVertexCounts:counts,limits:'Semantic rigid-plate retrofit with blended underarmor/cape; not animator-authored weights. Existing animations retained exactly; runtime device frame rate must be measured.'};
