...
async function fixture(t,dependencies={}){
 ...
 for(const [file,constant,url]of [['dungeon-assets','DUNGEON_MANIFEST_URL','/assets/dungeon/v13/manifest.json'],['prop-assets','PROP_MANIFEST_URL','/assets/props/v2/manifest.json'],['shrine-assets','SHRINE_MANIFEST_URL','/assets/props/v2/manifest.json'],['architecture-assets','ARCHITECTURE_MANIFEST_URL','/assets/props/v2/manifest.json']])await write(`app/game/${file}.ts`,`export const ${constant}='${url}';`);
...
test('mixed versions and exact GLB dependencies survive; only generated dungeon/prop legacy files are removed',async t=>{
 ...
 assert.equal(plan.manifests.length,2);assert.equal(plan.remove.length,4);assert.ok(plan.keep.some(k=>k.url==='/assets/props/v1/sconce.glb'));assert.ok(plan.keep.some(k=>k.url==='/assets/dungeon/v12/throne.glb'));assert.ok(plan.keep.some(k=>k.url==='/assets/dungeon/v9/passage-0.glb'));assert.ok(plan.keep.some(k=>k.url==='/assets/dungeon/v11/entry.png'));assert.ok(plan.keep.some(k=>k.url==='/assets/props/shared/mesh.bin'));assert.ok(plan.keep.some(k=>k.url==='/assets/shared/image.png'&&!k.prunableFamily));
...
test('each loader constant is resolved and all assets in every selected prop manifest form its closure',async t=>{
 const f=await fixture(t),other={version:'props-v3',assets:{shrine:{...f.props.assets.shrine},extra:{...f.props.assets.architecture}}};await f.write('dist/client/assets/props/v3/manifest.json',other);await f.write('app/game/shrine-assets.ts',"export const SHRINE_MANIFEST_URL='/assets/props/v3/manifest.json';");const plan=await planEnvironmentPrune(f.root);assert.equal(plan.manifests.length,3);assert.ok(plan.keep.some(k=>k.url==='/assets/props/v3/manifest.json'));await f.write('app/game/architecture-assets.ts','export const UNKNOWN=1;');await assert.rejects(planEnvironmentPrune(f.root),/Cannot resolve ARCHITECTURE_MANIFEST_URL/);await assertLegacyIntact(f);
});
