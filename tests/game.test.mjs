...
await build({stdin:{contents:'export { GET,PUT,POST } from "./app/api/progress/route"; export * from "./app/game/model"; export { GameEngine, stopCameraAtWall } from "./app/game/engine"; export * as THREE from "three"; export { createWorld, knight } from "./app/game/world"; export * from "./app/game/combat"; export { renderResolution, surfaceAsset } from "./app/game/graphics"; export * from "./app/game/dungeon";',resolveDir:root},bundle:true,format:'esm',platform:'node',outfile,logLevel:'silent',plugins:[{name:'test-d1',setup(b){b.onResolve({filter:/^@\/db\/game$/},()=>({path:'db',namespace:'test'}));b.onLoad({filter:/.*/,namespace:'test'},()=>({contents:'export function gameDb(){return globalThis.__gameTestDb}',loader:'js'}));}}]});
const {GET,PUT,POST,newProgress,progressSchema,GameEngine,stopCameraAtWall,THREE:T,createWorld,knight,AttackSequence,renderResolution,surfaceAsset,ROOMS,CORRIDORS,GATES,gateOpen,walkable,dungeonMove,validDungeonSpawn}=await import(pathToFileURL(outfile).href);
let sql;
...
test('World generation has finite geometry and reachable shrine/checkpoint positions',()=>{const original=T.TextureLoader.prototype.load;T.TextureLoader.prototype.load=function(){return new T.Texture()};try{const w=createWorld(newProgress(),'low');let meshes=0;w.scene.traverse(o=>{if(!o.isMesh)return;meshes++;const a=o.geometry.attributes.position.array;for(let i=0;i<a.length;i++)assert.ok(Number.isFinite(a[i]));});assert.ok(meshes>100);for(const point of [{x:0,z:51},...w.shrines.map(s=>({x:s.group.position.x,z:s.group.position.z}))])assert.ok(!w.colliders.some(c=>Math.hypot(c.x-point.x,c.z-point.z)<c.r+.6),'objective intersects a collider');}finally{T.TextureLoader.prototype.load=original;}});
